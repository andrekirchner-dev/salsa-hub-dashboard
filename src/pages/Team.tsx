...
interface TeamMember { id: string; name: string; role: string; email: string; avatarUrl?: string; }
interface TeamProject { id: string; name: string; status: string; }
interface TeamTask { id: string; title: string; status: string; assigneeName?: string; }
interface Team { id: string; name: string; description?: string; avatar: string; }

interface SectionProps {
  id: string; icon: React.ElementType; title: string;
  badge?: number; open: boolean; onToggle: (id: string) => void; children: React.ReactNode;
}
...
export default function Team() {
  ...
  const handleCreateTeam = async () => {
    ...
    await addDoc(collection(db, "teams"), {
      name: newTeamName.trim(), description: newTeamDesc.trim() || null,
      createdBy: currentUserId, createdAt: serverTimestamp(),
    });
    setNewTeamName(""); setNewTeamDesc(""); setSavingTeam(false); setTeamDialogOpen(false);
    loadTeams();
  };

  const handleRemoveMember = async (memberId: string) => {
    if (!selectedTeamId) return;
    await deleteDoc(doc(db, "teams", selectedTeamId, "members", memberId));
              ...
              <div className="space-y-3 mt-2">
                <div className="space-y-1">
                  ...
                  <Input value={newTeamName} onChange={(e) => setNewTeamName(e.target.value)}
                    placeholder="Ex: Time de Marketing" className="bg-surface-mid border-surface-high rounded-2xl" />
                </div>
                <div className="space-y-1">
                  ...
                  <Input value={newTeamDesc} onChange={(e) => setNewTeamDesc(e.target.value)}
                    placeholder="Breve descrição do time" className="bg-surface-mid border-surface-high rounded-2xl" />
                </div>
                <Button onClick={handleCreateTeam} disabled={savingTeam || !newTeamName}
          ...
          {selectedTeam && (
            <div>
              ...
              <DrawerSection id="members" icon={Users} title="Membros" badge={members.length} open={openSections.includes("members")} onToggle={toggleSection}>
                <div className="space-y-2 mb-3">
                  {members.length === 0 ? <p className="text-sm text-muted-foreground text-center py-2">Nenhum membro ainda.</p> : members.map((m) => (
                    <div key={m.id} className="flex items-center gap-3 p-3 rounded-2xl bg-surface-mid">
                      ...
                      {canManageTeam(currentUserRole) && m.id !== currentUserId && (
                ...
                {canManageTeam(currentUserRole) && (
                  <Dialog open={inviteDialogOpen} onOpenChange={setInviteDialogOpen}>
                    <DialogTrigger asChild>
                      <Button variant="outline" size="sm" className="rounded-2xl border-primary/50 text-primary w-full gap-2">
                    ...
                    <DialogContent className="bg-surface-low border-surface-mid rounded-3xl mx-4 max-w-sm">
                      ...
                      <div className="space-y-3 mt-2">
                        <p className="text-sm text-muted-foreground">O colaborador deve criar sua conta e compartilhar o UID com você para ser adicionado ao time.</p>
                        <div className="space-y-1">
                          <Label className="text-xs text-muted-foreground uppercase tracking-wide">Cargo no time</Label>
                          <div className="flex flex-wrap gap-2">
                            {ALL_ROLES.map((r) => (
                              <button key={r} onClick={() => setInviteRole(r)}
                                className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${inviteRole === r ? "bg-primary text-background" : "bg-surface-mid text-muted-foreground hover:bg-surface-high"}`}>{r}</button>
                            ))}
                          </div>
                        </div>
                        <Button className="w-full bg-primary text-background rounded-2xl font-semibold" onClick={() => setInviteDialogOpen(false)}>Fechar</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                )}
              ...
              <DrawerSection id="projects" icon={FolderKanban} title="Projetos" badge={projects.length} open={openSections.includes("projects")} onToggle={toggleSection}>
                {projects.length === 0 ? <p className="text-sm text-muted-foreground text-center py-2">Nenhum projeto vinculado a este time.</p> : (
                          ...
                          <div className="space-y-2">
                            {items.map((t) => (
                              <div key={t.id} className="p-3 rounded-2xl bg-surface-mid flex items-center justify-between gap-2">
                                ...
                                <span className="text-xs text-muted-foreground flex-shrink-0">{t.assigneeName}</span>
                              </div>
                            ))}
                          </div>
                        </div>
              ...
              {canManageTeam(currentUserRole) && (
                <DrawerSection id="settings" icon={Settings} title="Configurações" open={openSections.includes("settings")} onToggle={toggleSection}>
                  <div className="space-y-3">
                    ...
                    {canFullControl(currentUserRole) && (
                      <button className="w-full py-2.5 text-sm text-red-400 border border-red-500/30 rounded-2xl hover:bg-red-500/10 transition-colors mt-1">Excluir Time</button>
                    )}
                  </div>
                </DrawerSection>
              )}
