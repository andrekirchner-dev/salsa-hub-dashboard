...
export default function Library() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      getDoc(doc(db, "profiles", user.uid)).then((snap) => setUserRole(snap.data()?.role || ""));
    }
    loadTools();
  }, [loadTools]);
  ...
  const handleAdd = async () => {
    ...
    await addDoc(collection(db, "toolsLibrary"), {
      name: newName.trim(), toolFunction: newFunction.trim(), type: newType, url,
      addedBy: currentUserId, validated: true, createdAt: serverTimestamp(),
    });
    setNewName(""); setNewFunction(""); setNewType("Site"); setNewUrl("");
    setSaving(false); setDialogOpen(false); loadTools();
  };

  const handleDelete = async (id: string) => { await deleteDoc(doc(db, "toolsLibrary", id)); loadTools(); };

  const filtered = tools.filter((t) => {
    const matchSearch = t.name.toLowerCase().includes(search.toLowerCase()) || t.toolFunction.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-background pb-24 px-4 pt-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate("/")} className="w-9 h-9 rounded-2xl bg-surface-low border border-surface-mid flex items-center justify-center hover:bg-surface-mid transition-colors">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {TOOL_TYPES.map((t) => (
                      <button key={t} onClick={() => setNewType(t)}
                        className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${newType === t ? "bg-primary text-background" : "bg-surface-mid text-muted-foreground hover:bg-surface-high"}`}>{t}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-1">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-none">
        {allTypes.map((t) => (
          <button key={t} onClick={() => setFilterType(t)}
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${filterType === t ? "bg-primary text-background" : "bg-surface-low border border-surface-mid text-muted-foreground hover:bg-surface-mid"}`}>{t}</button>
        ))}
      </div>
      ...
      <div className="bg-surface-low border border-surface-mid rounded-3xl overflow-hidden">
        {loading ? (
          ...
          <div className="overflow-hidden">
            ...
            {filtered.map((tool, i) => (
              <div key={tool.id}
                className={`grid grid-cols-[1fr_1fr_auto_auto] gap-0 items-center px-4 py-3 ${i !== filtered.length - 1 ? "border-b border-surface-mid" : ""} hover:bg-surface-mid/40 transition-colors group`}>
